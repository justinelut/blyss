...
const formatCurrencyCompact = (
  ...
  const effectiveLocale =
    locales || (currency.toLowerCase() === 'kes' ? 'en-KE' : undefined)
  const currencyNumberFormat = new Intl.NumberFormat(effectiveLocale, {
    style: 'currency',
    currency,
 ...
 * @param locales - Optional locale specification. If undefined, uses 'en-KE' for KES currency, otherwise browser's default locale.
 *                  Use explicit locales only when you need consistent formatting across different environments.
 *
 * @returns A function that takes cents and currency (defaults to 'kes') and returns the formatted string
 *
 * @example
 ...
 * formatCompact(12345, 'kes') // Returns: "KSh 123.45"
 * formatCompact(12300, 'kes') // Returns: "KSh 123" (hides .00)
 * formatCompact(12345) // Returns: "KSh 123.45" (defaults to KES)
 ...
 * formatStandard(12345, 'kes') // Returns: "KSh 123.45"
 ...
 * formatAccounting(12345, 'kes') // Returns: "KSh 123.45"
 * formatAccounting(12300, 'kes') // Returns: "KSh 123.00" (always shows decimals)
 ...
 * formatStatistics(12345, 'kes') // Returns: "KSh 123.5"
 * formatStatistics(4200000, 'kes') // Returns: "KSh 42K"
 ...
 * formatSubcent(1, 'kes') // Returns: "KSh 0.01"
...
export const formatCurrency =
  ...
  (cents: number, currency: string = 'kes'): string => {
    switch (mode) {
      case 'compact':
