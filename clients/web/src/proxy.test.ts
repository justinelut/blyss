...
describe("middleware function", () => {
  ...
  it("serves the unprefixed canonical URL directly to search crawlers", async () => {
    const request = new NextRequest("https://example.com/marketplace", {
      headers: {
        "user-agent":
    ...
    expect(response.headers.get("x-middleware-request-x-blyss-country")).toBe(
      "ke",
    );
    expect(response.headers.get("x-middleware-request-x-blyss-currency")).toBe(
      "kes",
    );
  });

  it("permanently redirects www to the canonical production host", async () => {
    const request = new NextRequest(
      "https://www.blyss.co.ke/ke/marketplace?sort=newest",
      { headers: { host: "www.blyss.co.ke" } },
    );
    ...
    expect(response.headers.get("location")).toBe(
      "https://blyss.co.ke/ke/marketplace?sort=newest",
    );
  });
  ...
  it("rewrites a localized Kenya product URL without changing country", async () => {
    const request = new NextRequest("https://example.com/ke/product/prod_123");
  ...
  it("should redirect to login with query params preserved", async () => {
    ...
    expect(location).toContain(
      "return_to=%2Fdashboard%3Ffoo%3Dbar%26baz%3Dqux",
    );
  });
