...
describe("Creator categories", () => {
  ...
  test("Directory filters server-provided categories on the real creator_category field", () => {
    const page = read("src/components/Marketplace/CreatorsDirectoryPage.tsx");
    expect(page).toContain("const categories = initialCategories");
    expect(page).not.toContain("useCreatorCategories");
