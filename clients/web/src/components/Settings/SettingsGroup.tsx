...
export const SettingsGroup: React.FC<React.PropsWithChildren> = ({
  ...
  <div className="dark:ring-polar-700 dark:bg-polar-900 dark:divide-polar-700 w-full flex-col divide-y divide-[var(--border)] overflow-hidden rounded-2xl bg-transparent ring-1 ring-[var(--border)] dark:ring-1">
    {children}
  </div>
)
...
> = ({ children, title, description, vertical }) => (
  ...
  >
    <div className="flex w-full flex-col md:max-w-1/2">
      ...
      {description && (
        <p className="dark:text-polar-500 text-xs text-[var(--text-muted)]">
          {description}
        </p>
      )}
    </div>
