...
export const StartStatsStrip = ({ stats }: Props) => {
  ...
  const moneyValue =
    stats.total_paid_out > 0 ? stats.total_paid_out : stats.total_earned;
  const moneyLabel =
    stats.total_paid_out > 0 ? "Paid to creators" : "Earned by creators";
  const moneyDetail =
    ...
    >
      <div className="mx-auto max-w-[1280px] px-6 py-12 md:px-16 md:py-16">
        ...
        <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-4">
          ...
          {showMoney && (
            <Stat
              value={formatMoney(moneyValue, stats.total_paid_out_currency)}
              label={moneyLabel}
              detail={moneyDetail}
            />
          )}
