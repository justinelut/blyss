...
const ClientPage: React.FC<ClientPageProps> = ({
  ...
  const columns: DataTableColumnDef<schemas['Order']>[] = [
    ...
    {
      accessorKey: 'payment_processor',
      enableSorting: false,
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="Processor" />
      ),
      cell: ({ row: { original: order } }) => {
        // Determine payment processor based on platform_fee_amount
        const isPaystack =
          order.platform_fee_amount && order.platform_fee_amount > 0
        return (
          <Status
            status={isPaystack ? 'Paystack' : 'Stripe'}
            className={
              isPaystack
                ? 'bg-green-100 text-xs text-green-600 dark:bg-green-950'
                : 'bg-blue-100 text-xs text-blue-600 dark:bg-blue-950'
            }
          />
        )
      },
