...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    const creatorName = org?.name?.trim();
    const title = creatorName ? `${p.name} by ${creatorName}` : p.name;
    // Anti-slop fallback: when a creator hasn't filled in their product
    // description, name the product, payment rail, and delivery behavior.
    const desc =
      p.description?.slice(0, 160) ||
      (creatorName
        ? `${p.name} by ${creatorName}. Buy with M-Pesa or card. Instant download after payment.`
        : `${p.name}. Buy with M-Pesa or card. Instant download after payment.`);
...
export default async function ProductPage({ params }: Props) {
  ...
  try {
    product = await fetchProduct(id, visitorCurrency);
  } catch {
    notFound();
  }
  ...
  const price =
    prices.find(
      (p) =>
        (p?.price_currency ?? "").toLowerCase() ===
        visitorCurrency.toLowerCase(),
    ) ?? prices[0];
  const amount = (price as any)?.price_amount ?? 0;
  const currency = (
    (price as any)?.price_currency ?? visitorCurrency
  ).toUpperCase();
  ...
  const productLd: Record<string, unknown> = {
    ...
    image:
      product.medias?.filter((m) => m.public_url).map((m) => m.public_url) ??
      [],
    sku: product.id,
  ...
  const reviewAvg = (product as any).review_rating_avg as
    | number
    | null
    | undefined;
  if (
