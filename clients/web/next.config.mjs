...
// MinIO self-hosted storage endpoints
const MINIO_CONSOLE = 'https://storage.blyss.co.ke' // Console UI
const MINIO_API = 'https://s3.blyss.co.ke' // API for uploads
...
const nextConfig = {
  ...
  webpack: (config, { dev, isServer }) => {
    // Completely disable all caching in development to prevent compaction issues
    if (dev) {
      config.cache = false
  ...
  images: {
    remotePatterns: [
      ...
      {
        protocol: 'https',
        hostname: 's3.blyss.co.ke',
        port: '',
        pathname: '**',
      },
    ],
    formats: ['image/avif', 'image/webp'],
    // Reduce cache TTL in development, use 1 year in production
    minimumCacheTTL: ENVIRONMENT === 'development' ? 60 : 31536000,
