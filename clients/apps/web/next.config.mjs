...
// Cloudflare R2 endpoint for file uploads
const R2_ENDPOINT =
  'https://c1eaaa292b9dddcb67f9592bb5bc1948.r2.cloudflarestorage.com'

// MinIO self-hosted storage endpoint
const MINIO_ENDPOINT = 'https://storage.blyss.co.ke'

const baseCSP = `
    default-src 'self';
    connect-src 'self' ${process.env.NEXT_PUBLIC_API_URL} ${process.env.S3_UPLOAD_ORIGINS} ${R2_ENDPOINT} ${MINIO_ENDPOINT} https://api.stripe.com https://maps.googleapis.com https://*.google-analytics.com https://chat.uk.plain.com https://prod-uk-services-attachm-attachmentsuploadbucket2-1l2e4906o2asm.s3.eu-west-2.amazonaws.com;
...
const nextConfig = {
  ...
  webpack: (config, { dev, isServer }) => {
    // Disable filesystem cache on Windows in development to prevent compaction issues
    if (dev && process.platform === 'win32') {
      config.cache = false
    } else if (config.cache && !dev) {
