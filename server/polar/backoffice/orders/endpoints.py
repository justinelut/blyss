...
class PaymentProcessorItem(description_list.DescriptionListItem[Order]):
    def __init__(self):
        super().__init__("Payment Processor")

    def render(self, request: Request, item: Order) -> Generator[None] | None:
        # Determine payment processor type based on order characteristics
        if item.platform_fee_amount > 0:
            processor_type = "Paystack"
            transaction_ref = item.stripe_invoice_id
            with tag.div():
                with tag.span(classes="badge badge-info"):
                    text(processor_type)
                if transaction_ref:
                    with tag.div(classes="text-sm text-gray-600 mt-1"):
                        text(f"Transaction: {transaction_ref}")
        else:
            ...
            with tag.div():
                with tag.span(classes="badge badge-primary"):
                    text(processor_type)
                if stripe_invoice_id and stripe_invoice_id.startswith("in_"):
                    with tag.div(classes="text-sm mt-1"):
                        with tag.a(
                            href=f"https://dashboard.stripe.com/invoices/{stripe_invoice_id}",
                            target="_blank",
                            classes="link",
                        ):
                            text(f"Invoice: {stripe_invoice_id}")
        return None
