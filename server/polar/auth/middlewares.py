...
async def get_auth_subject(
    ...
    # Check for guest session token in custom header
    guest_session_token = request.headers.get("X-Guest-Session-Token")
    if guest_session_token:
        guest_session = GuestSession(guest_session_token)
        return AuthSubject(Anonymous(), set(), guest_session)
