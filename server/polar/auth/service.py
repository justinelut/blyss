...
class AuthService:
    async def get_login_response(
        ...
        *,
        ...
        guest_session_token = self._extract_guest_session_token(request)

        if guest_session_token:
            from polar.cart.service import cart as cart_service

            try:
                migrated_count = await cart_service.migrate_guest_cart(
                    session=session,
                ...
                if migrated_count > 0:
                    log.info(
                        "Migrated guest cart on login",
                        user_id=user.id,
                        migrated_count=migrated_count,
                    )
            except Exception as e:
                log.warning(
                    "Failed to migrate guest cart on login",
                    user_id=user.id,
                    error=str(e),
                )
        ...
        if guest_session_token:
            response = self._clear_guest_session_cookie(request, response)
    ...
    def _extract_guest_session_token(self, request: Request) -> str | None:
        """Extract guest session token from cookies."""
        token = request.cookies.get(settings.GUEST_SESSION_COOKIE_KEY)
        if token and token.isascii():
            return token
        return None

    def _clear_guest_session_cookie(self, request: Request, response: R) -> R:
        """Clear the guest session cookie after migration."""
        is_localhost = request.url.hostname in {"127.0.0.1", "localhost"}
        secure = False if is_localhost else True
        response.set_cookie(
            settings.GUEST_SESSION_COOKIE_KEY,
            value="",
            expires=0,
            path="/",
            domain=settings.USER_SESSION_COOKIE_DOMAIN,
