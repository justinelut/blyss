...
class Settings(BaseSettings):
    ...
    GUEST_SESSION_COOKIE_KEY: str = "polar_guest_session"
    ...
    POSTGRES_READ_SSL_MODE: str | None = None
    ...
    def get_postgres_dsn(self, driver: Literal["asyncpg", "psycopg2"]) -> str:
        ...
        # Add SSL mode for Neon and other managed PostgreSQL services
        if self.POSTGRES_SSL_MODE:
            # psycopg2 uses 'sslmode', asyncpg uses 'ssl'
            ssl_param = "sslmode" if driver == "psycopg2" else "ssl"
            dsn += f"?{ssl_param}={self.POSTGRES_SSL_MODE}"
            # Add connection timeout for asyncpg (120 seconds for slow connections)
            if driver == "asyncpg":
                dsn += "&command_timeout=120"
        return dsn
    ...
    def get_postgres_read_dsn(
        ...
        # Add SSL mode for Neon and other managed PostgreSQL services
        if self.POSTGRES_READ_SSL_MODE:
            # psycopg2 uses 'sslmode', asyncpg uses 'ssl'
            ssl_param = "sslmode" if driver == "psycopg2" else "ssl"
            dsn += f"?{ssl_param}={self.POSTGRES_READ_SSL_MODE}"
            # Add connection timeout for asyncpg (120 seconds for slow connections)
            if driver == "asyncpg":
                dsn += "&command_timeout=120"
        return dsn
    ...
    def validate_paystack_config(self) -> None:
        """Validate that required Paystack environment variables are set.

        Only validates if at least one Paystack variable is set, allowing
        for gradual migration and development without Paystack configured.
        """
        paystack_vars = {
        ...
        # Check if any Paystack variable is set
        any_set = any(var_value for var_value in paystack_vars.values())

        if not any_set:
            # No Paystack variables set - skip validation
            # This allows development without Paystack configured
            return

        # If any variable is set, all must be set
        missing_vars = [
            var_name for var_name, var_value in paystack_vars.items() if not var_value
        ]
        ...
        if missing_vars:
            ...
            log.error(
                "paystack.config.missing_required_variables",
                missing_variables=missing_vars,
            )
            raise ValueError(
                f"Missing required Paystack environment variables: {', '.join(missing_vars)}. "
                f"Either set all Paystack variables or none to disable Paystack integration."
            )

    def validate_platform_fee_config(self) -> None:
        """Validate that platform fee configuration is within acceptable range.

        Ensures PLATFORM_FEE_BASIS_POINTS is non-negative and reasonable.
        """
