...
try:
    ...
    class FcntlCompat:
        """Windows-compatible fcntl wrapper using msvcrt"""
        ...
        def flock(fd: int, operation: int) -> None:
            ...
            if operation & FcntlCompat.LOCK_UN:
                # Unlock
                try:
                    msvcrt.locking(fd, msvcrt.LK_UNLCK, 1)
                except OSError:
                    pass
            elif operation & FcntlCompat.LOCK_EX:
                # Exclusive lock
                if operation & FcntlCompat.LOCK_NB:
                    # Non-blocking
                    try:
