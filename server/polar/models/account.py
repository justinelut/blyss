...
class Account(RecordModel):
    ...
    def calculate_fee_in_cents(self, amount_in_cents: int) -> int:
        # Validate input amount is non-negative
        if amount_in_cents < 0:
            log.error(
                "fee_calculation.negative_amount",
                amount_in_cents=amount_in_cents,
                account_id=self.id,
            )
            raise ValueError(
                f"Transaction amount must be non-negative, got {amount_in_cents}"
            )
        ...
        calculated_fee = polar_round(fee_in_cents)

        # Validate calculated fee is non-negative
        if calculated_fee < 0:
            log.error(
                "fee_calculation.negative_fee",
            ...
            raise ValueError(
                f"Calculated fee must be non-negative, got {calculated_fee}"
            )

        # Validate fee doesn't exceed transaction amount
        if calculated_fee > amount_in_cents:
            log.error(
                "fee_calculation.fee_exceeds_amount",
                calculated_fee=calculated_fee,
                amount_in_cents=amount_in_cents,
                basis_points=basis_points,
                fixed=fixed,
            ...
            raise ValueError(
                f"Calculated fee {calculated_fee} exceeds transaction amount {amount_in_cents}"
            )
