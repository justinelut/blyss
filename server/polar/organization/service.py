...
from polar.integrations.paystack.logging import log_subaccount_creation_failure
...
class UnauthorizedProfileUpdate(OrganizationError):
    def __init__(self, organization_id: UUID) -> None:
        ...
        message = (
            f"You do not have permission to update organization {organization_id}."
        )
        super().__init__(message, 403)
...
class OrganizationService:
    ...
    async def create_organization_subaccount(
        ...
        Args:
            ...
            organization: Organization to create subaccount for

        Returns:
            Updated organization with subaccount details

        Raises:
            OrganizationError: If subaccount creation fails
        """
        log.info(
        ...
        try:
            # Call PaystackService to create subaccount
            # Configure with 20% platform fee (80% goes to creator)
            ...
            organization = await repository.update(
                ...
                update_dict={
                    ...
                    "subaccount_status": subaccount_data["status"],
                },
            )
            ...
            # Update organization with failed status
            repository = OrganizationRepository.from_session(session)
            organization = await repository.update(
                organization,
                update_dict={
            ...
            raise OrganizationError(
                f"Failed to create Paystack subaccount for organization {organization.id}: {e}"
            )

    def uses_paystack(self, organization: Organization) -> bool:
        """
    ...
    async def update_creator_profile(
        ...
        Args:
            session: Database session
            auth_subject: Authenticated user or organization
        ...
        Raises:
            NotPermitted: If user doesn't have permission to update organization
        """
        repository = OrganizationRepository.from_session(session)
        ...
        readable_org = await repository.get_one_or_none(statement)
