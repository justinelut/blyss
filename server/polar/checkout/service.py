...
class CheckoutService:
    ...
    async def create(
        ...
        if isinstance(checkout_create, CheckoutPriceCreate):
        ...
        elif isinstance(checkout_create, CheckoutCartCreate):
            # Handle cart-based checkout
            products, cart_items = await self._get_validated_cart_items(
                session, auth_subject, checkout_create.cart_items
            )
            product = products[0]
            ...
            # For cart checkout, we'll use a synthetic price representing the combined total
            # The actual order items will be created from individual cart items
            try:
                price_set = PriceSet.from_product(product, *currencies)
                ...
                raise PolarRequestValidationError(
                    [
                        {
                            "type": "value_error",
                            "loc": ("body", "cart_items", 0),
                            "msg": "Products are not available in the specified currency.",
                            "input": checkout_create.cart_items[0],
                        }
                    ]
                ) from e
        else:
    ...
    async def _clear_cart_items_after_checkout(
        self,
        session: AsyncSession,
        checkout: Checkout,
        cart_items: list[CartItem],
    ...
    async def _get_validated_cart_items(
        self,
        session: AsyncSession,
        auth_subject: AuthSubject[User | Organization],
        cart_item_ids: list[UUID4],
    ) -> tuple[list[Product], list[CartItem]]:
        """Get and validate cart items for checkout."""
        from polar.cart.repository import CartRepository
                    ...
                    [
                        {
                            "type": "value_error",
                            "loc": ("body", "cart_items"),
                            "msg": f"Cart item {cart_item_id} not found.",
                            "input": str(cart_item_id),
                        }
                    ]
                )
            cart_items.append(cart_item)

        # Extract products and validate they belong to the same organization
        products = []
                    ...
                    [
                        {
                            "type": "value_error",
                            "loc": ("body", "cart_items"),
                            "msg": "All cart items must belong to the same organization.",
                            "input": cart_item_ids,
                        }
                    ]
                )

            # Validate product is not recurring (cart only supports one-time products)
            if product.is_recurring:
                raise PolarRequestValidationError(
                    [
                        {
                            "type": "value_error",
                            "loc": ("body", "cart_items"),
                            "msg": f"Cart item {cart_item.id} contains a recurring product. Only one-time products are supported in cart checkout.",
                    ...
                    [
                        {
                            "type": "value_error",
                            "loc": ("body", "cart_items"),
                            "msg": f"Cart item {cart_item.id} contains an archived product.",
                            "input": str(cart_item.id),
                        }
                    ]
                )
...
from polar.cart.service import cart as cart_service

checkout = CheckoutService(cart_service)
